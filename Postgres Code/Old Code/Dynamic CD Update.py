...
origional_class_words = pickle.load( open( "class_words_0.pickle", "rb" ) )
class_words = copy.deepcopy(origional_class_words)

# Load a data frame of all the test data
test_DFrame = pd.read_csv('test_set_0_keywords.csv')
...
# This list is used to keep track of how many times a keyword's tail has been
#    extended. At the very begining of the extension code we will loop through
#    and set this equal to 2 for each node of class words. 
track_tails_extended = []
...
# Initialize the track_tails_extended list here:
it1 = 0
for x in class_words:
    # Will need to add an empty list for every category
    track_tails_extended.append([])
    # Now loop through all of the words in the current category and append
    #    a '2' to the inner list if the thing is a keyword, '0' otherwise
    for y in class_words[x]:
        if class_words[x][y][0] > 0:
            track_tails_extended[it1].append(2)
        else:
            track_tails_extended[it1].append(0)
    it1 = it1 + 1

# Now that we have the probabilities we can start checking keyword probabilities against
#   thresholds.

# We will use class_words to determine which indexes in kde_lists_norm are keywords.
# Iterate through all of the dictionaries in class_words:

# This boolean is used to keep extending the tails until this flag is tripped
done_extending = False

while not done_extending:
    # This iterator will be used to keep track of which category we are in (because this 
    #    number is different than x in the for loop). We will use it1 and it2 to interact
    #    with the track_tails_extended and kde_lists_norm lists.
    it1 = 0 
    # This boolean is used to tripped if there is a change made during a threshold check of class_words
    change_made = False
    print("Starting threshold check number:", threshold_iteration ) 
    for x in origional_class_words:
        # This counter will be incremented each time we extend a tail and output to terminal after every category. 
        count = 0
        ...
        for y in origional_class_words[x]:
            ...
            if origional_class_words[x][y][0] > 0:
                ...
                # Now we just check if this specific keyword's probability is above the threshold
                # and we use the track_tails_extended list to see which threshold value to test against.
                
                # This length check is because we don't update the length of track_tails_extended. We do not need to 
                #    update the length of it because only non keywords are added so we can just continue if the it2
                #    is longer than the len of track_tails_extended[it1]
                if it2 > len(track_tails_extended[it1]):
                    continue
                elif track_tails_extended[it1][it2] == 2:
                    if kde_lists_norm[it1][it2] > two_th:
                        ...
                        if len(edge_results) > 0:
                            for z in edge_results: 
                                ...
                                count = count + 1
                        # Don't forget to update track_tails_extended & update change_made
                        track_tails_extended[it1][it2] = track_tails_extended[it1][it2] + 1
                        change_made = True
                elif track_tails_extended[it1][it2] == 3:
                    if kde_lists_norm[it1][it2] > three_th:
                        # Now extend the tail of this keyword out by 1
                        # First we need the node id of the current keyword, y. Right now y is an uri and
                        #    we cannot pass in uri's to query_node, so we will use split() to get just the word.
                        keyword_results = query_node(y.split('/')[3])
                        ...
                        track_tails_extended[it1][it2] = track_tails_extended[it1][it2] + 1
                        change_made = True
                elif track_tails_extended[it1][it2] == 4:
                    if kde_lists_norm[it1][it2] > four_th:
                        # Now extend the tail of this keyword out by 1
                        # First we need the node id of the current keyword, y. Right now y is an uri and
                        #    we cannot pass in uri's to query_node, so we will use split() to get just the word.
                        keyword_results = query_node(y.split('/')[3])
    ...
    # TODO I just copied and pasted this code, put it in a function, its gross like this. I'm also not generating a new acc score each time, but I could. 
    # Create a 2d list containing a list of probabilities for each category.
    # The first dimension is the category, the second is a list of all words in that category. 
    # So the first entry of the first list would be the probability of that uri.  
    ...
    # Perform the done_extending check
    if not change_made:
        done_extending = True
    
    # Don't forget to incrementing this counter, useful to output.
    threshold_iteration = threshold_iteration + 1

    # Need to update the origional_class_words object after we are done making all of the changes to class_words. 
    origional_class_words = copy.deepcopy(class_words)
...
for x in results_list:
    ...
    i = i + 1
initial_accuracy_score = temp/len(results_list)
print("The new accuracy score for this model is:", initial_accuracy_score)
# TODO end copy here

# Now pickle the class_words dict. 
print("Creating pickle for the extended class_words dict.")
pickle.dump( class_words, open("extended_class_words_0.P", "wb") )
